...
Certain endpoints may require an authentication token, which is obtained after registering and logging.
	...
	2. Attempt login with the 'login' endpoint. The api will return a JWT response:
		Request:
		```
			curl -X 'POST' \
	...
	3. Request for an endpoint that needs autorization: add the received token alongside the request.
		```
		curl -X 'GET' \
